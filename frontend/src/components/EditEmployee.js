...
const EditEmployee = ({ user, onLogout }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        ...
        setExistingImage(employee.f_Image);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.f_Image && existingImage) {
      formDataToSend.append('f_Image', existingImage);
    }

    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/employees/update`, formDataToSend);
  ...
  return (
    <div className="edit-employee-container">
      ...
      <form onSubmit={handleSubmit}>
        <div>
          <label>ID</label>
          <input type="number" value={formData.f_Id} disabled />
        </div>
        <div>
          <label>Name</label>
          <input
        ...
        <div>
          ...
          {existingImage && <img src={`${process.env.REACT_APP_API_URL}/${existingImage}`} alt="current" width="100" />}
          <input
            type="file"
